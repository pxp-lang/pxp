...
impl<'a> Parser<'a> {
    ...
    pub(crate) fn expected_token(&mut self, kind: TokenKind) {
        self.diagnostic(ParserDiagnostic::ExpectedToken { expected: vec![kind], found: self.current().to_owned() }, Severity::Error, self.current_span());
    }

    pub(crate) fn expected_any_of_tokens(&mut self, kinds: &[TokenKind]) {
        self.diagnostic(ParserDiagnostic::ExpectedToken { expected: kinds.to_vec(), found: self.current().to_owned() }, Severity::Error, self.current_span());
    }
