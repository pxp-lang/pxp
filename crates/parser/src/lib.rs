...
impl<'a> Parser<'a> {
    ...
    fn top_level_statement(&mut self) -> Statement {
            ...
            TokenKind::Namespace | TokenKind::Use | TokenKind::Const | TokenKind::HaltCompiler => {
                ...
                let kind = match &self.state.current().kind {
                    ...
                    TokenKind::HaltCompiler => {
                        ...
                        let (span, content) = if let TokenKind::InlineHtml = self.state.current().kind {
                            let content = self.state.current().clone();
                            state.next();
        ...
        let statement = if has_attributes {
            ...
            match &current.kind {
                ...
                TokenKind::Declare => {
                    ...
                    let body = match self.state.current().kind {
                        TokenKind::SemiColon => {
                ...
                TokenKind::Static if matches!(peek.kind, TokenKind::Variable) => {
                    ...
                    loop {
                        ...
                        if self.state.current().kind == TokenKind::Equals {
                            state.next();
                ...
                TokenKind::InlineHtml => {
                    let html = self.state.current().to_owned();
                    state.next();
