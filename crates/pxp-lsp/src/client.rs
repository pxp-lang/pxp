...
use lsp_types::{notification::{LogMessage, Notification as NotificationTrait, Progress, PublishDiagnostics}, request::{Request as _, WorkDoneProgressCreate}, Diagnostic, LogMessageParams, MessageType, ProgressParams, ProgressParamsValue, ProgressToken, PublishDiagnosticsParams, Uri, WorkDoneProgress, WorkDoneProgressBegin, WorkDoneProgressCreateParams, WorkDoneProgressEnd, WorkDoneProgressReport};
...
impl Client<'_> {
    ...
    pub fn with_progress(&self, title: impl Into<String>, cb: impl FnOnce(&ProgressReporter) -> Result<(), Box<dyn Error + Sync + Send>>) -> Result<(), Box<dyn Error + Sync + Send>> {
...
impl<'a> ProgressReporter<'a> {
    ...
    pub fn report(&self, progress: u32, message: Option<String>) -> Result<(), Box<dyn Error + Sync + Send>> {
        self.client.connection.sender.send(Message::Notification(Notification {
            method: Progress::METHOD.to_string(),
            params: to_value(ProgressParams {
