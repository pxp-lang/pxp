...
use pxp_lsp::types::{
    CompletionItem, CompletionItemKind, CompletionItemLabelDetails, Position, Uri,
};
...
impl Backend {
    pub(crate) fn get_completion_items(
        &mut self,
        uri: &Uri,
        position: Position,
        ...
        let Some((node, ancestors)) =
            NodeFinder::find_at_byte_offset(&parse_result.ast, offset as usize)
        else {
            return Ok(items);
        };
        ...
        match completion_kind {
            CompletionKind::PropertyOrMethod => {
                complete_property_or_method(&node, &ancestors, &self.index, &map, &mut items)
            }
            CompletionKind::Extends => {
                complete_extends(&node, &ancestors, &self.index, &map, &mut items)
            }
            CompletionKind::StaticPropertyMethodOrConstant => {
                complete_static_property_method_or_constant(
                    &node,
                    &ancestors,
                    &self.index,
                    &map,
            ...
            CompletionKind::ContextualMethodName => {
                complete_contextual_method_names(&node, &ancestors, &map, &self.index, &mut items)
            }
...
fn complete_static_property_method_or_constant(
    node: &Node<'_>,
    ...
    // Grab the target.
    let target = constant_fetch
        .as_constant_fetch_expression()
        .unwrap()
        .target
        .as_ref();
...
fn complete_contextual_method_names(
    node: &Node,
    ancestors: &Ancestors,
    map: &TypeMap,
    index: &Index,
...
impl CompletionContext {
    ...
    fn enum_body(node: &Node, ancestors: &Ancestors) -> bool {
        node.is_unit_enum_body()
            || ancestors.find(|n| n.is_unit_enum_body()).is_some()
            || node.is_backed_enum_body()
            || ancestors.find(|n| n.is_backed_enum_body()).is_some()
    }
    ...
    fn not_missing_classish_member(node: &Node, ancestors: &Ancestors) -> bool {
        !node.is_missing_classish_member()
            && ancestors.find(|n| n.is_missing_classish_member()).is_none()
    }

    fn method_name(node: &Node, ancestors: &Ancestors) -> bool {
        node.is_abstract_method()
            || node.is_concrete_method()
            || (node.is_simple_identifier()
                && ancestors
                    .find(|n| n.is_abstract_method() || n.is_concrete_method())
                    .is_some())
    }
}
...
fn magic_methods(items: &mut Vec<CompletionItem>) {
    let methods = [
        ...
        (
            "__call",
            "(string $${1:name}, array $${2:arguments}): ${3:mixed}\n{$0\n}",
        ),
        (
            "__callStatic",
            "(string $${1:name}, array $${2:arguments}): ${3:mixed}\n{$0\n}",
        ...
        (
            "__set",
            "(string $${1:name}, mixed $${2:value}): void\n{$0\n}",
        ),
...
fn complete_keywords(
    ...
    if CompletionContext::method_name(node, ancestors) {
        magic_methods(items)
    } else if CompletionContext::classish_member(node, ancestors)
        && CompletionContext::not_missing_classish_member(node, ancestors)
    {
...
fn complete_extends(
    ...
    let Some(class_statement) = ancestors
        .find(|node| node.is_class_statement())
        .and_then(|node| node.as_class_statement())
    else {
        return;
    };
...
fn complete_property_or_method(
    node: &Node,
    ancestors: &Ancestors,
    index: &Index,
    map: &TypeMap,
    ...
    for candidate in candidates {
        ...
        for method in candidate.get_accessible_methods(scope.as_ref(), index) {
            items.push(CompletionItem {
                ...
                sort_text: if method.get_name().starts_with(b"__") {
                    Some("1".to_string())
                } else {
                    Some("0".to_string())
                },
...
fn get_reflection_classes(index: &Index, typ: &Type<ByteString>) -> Vec<ReflectionClass> {
    match typ {
        Type::Named(name) => {
            if let Some(class) = index.get_class(name) {
                vec![class]
            } else {
                vec![]
            }
        ...
        Type::Union(inner) => inner
            .iter()
            .flat_map(|t| get_reflection_classes(index, t))
            .collect(),
        Type::Intersection(inner) => inner
            .iter()
            .flat_map(|t| get_reflection_classes(index, t))
            .collect(),
...
fn completion_kind(node: &Node, ancestors: &Ancestors) -> CompletionKind {
    // $object->^
    if node.is_property_fetch_expression()
        || ancestors
            .find(|n| n.is_property_fetch_expression())
            .is_some()
    {
    ...
    // MyClass::^
    if node.is_simple_identifier()
        && ancestors
            .find(|n| n.is_constant_fetch_expression())
            .is_some()
    {
